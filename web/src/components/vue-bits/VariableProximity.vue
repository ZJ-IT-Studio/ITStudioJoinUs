...
const getGlobalLetterIndex = (wordIndex: number, letterIndex: number) => {
  let globalIndex = 0;
  for (let i = 0; i < wordIndex; i++) globalIndex += words.value[i].length;
...
const handleTouchMove = (ev: TouchEvent) => { if (ev.touches[0]) updatePosition(ev.touches[0].clientX, ev.touches[0].clientY); };

const animationLoop = () => {
  if (!props.containerRef) { animationFrameId = requestAnimationFrame(animationLoop); return; }
  const { x, y } = mousePositionRef;
  if (lastPositionRef.x === x && lastPositionRef.y === y) { animationFrameId = requestAnimationFrame(animationLoop); return; }
  ...
  letterRefs.value.forEach(letterEl => {
    ...
    // fallback: also brighten color for non-variable-font support
    const alpha = 0.4 + 0.6 * falloffValue;
    letterEl.style.color = `rgba(255,255,255,${alpha})`;
  });
...
<template>
  <span ref="rootRef" :class="[props.className]" :style="{ display: 'inline', ...props.style }" @click="props.onClick">
    <span v-for="(word, wordIndex) in words" :key="wordIndex" :style="{ display: 'inline-block', whiteSpace: 'nowrap' }">
      <span
        v-for="(letter, lIdx) in word.split('')" :key="getLetterKey(wordIndex, lIdx)"
